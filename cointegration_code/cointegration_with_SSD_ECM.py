...
Rf = pd.read_csv("../distance_data/Rf.csv", header=None) # Added for final analysis
...
no_pairs = MAX_PAIRS if TEST_MODE else 10
days, num_assets = np.shape(returns)
first_traininig = int(periods.iloc[0, 3])
Rpair = np.zeros((days, no_pairs))

asset_frequency_counter = Counter()
Rp_ew_cc = pd.DataFrame(np.zeros((days, 2)), columns=['Return', 'Semester']) # For Committed Capital
Rp_vw_fi = pd.DataFrame(np.zeros((days, 2)), columns=['Return', 'Semester']) # For Fully Invested
risk_free_df = pd.DataFrame(np.zeros((days, 2)), columns=['Return', 'Semester']) # For risk-free returns
...
if USE_SSD_ECM:
    ...
    ecm_dir = "../cointegration_results/ecm_results"
...
for big_loop in range(0, total_periods):
    ...
    try:
     ...
     # --- Processing and counting selected pairs ---
    if USE_SSD_ECM:
        if not coint_pairs_df.empty:
            coint_pairs_df = coint_pairs_df[coint_pairs_df['Status'] == 'OK'].copy()
            coint_pairs_df = coint_pairs_df.sort_values('SSD_value').head(no_pairs).copy()
            ...
            # CORRECTED PLACEMENT for asset counting in SSD_ECM mode
            print("Counting selected assets for this period...")
            for pair_str in coint_pairs_df['Pair']:
                s1, s2 = pair_str.split('-')
                asset_frequency_counter[s1] += 1
                asset_frequency_counter[s2] += 1
        ...
        try:
            ...
            # CORRECTED PLACEMENT for asset counting in ADF-only mode
            print("Counting selected assets for this period...")
            for idx, row in coint_pairs_df.iterrows():
                asset_frequency_counter[row['Stock A']] += 1
                asset_frequency_counter[row['Stock B']] += 1
            
        except Exception as e:
            print(f"Exception while selecting top pairs: {e}")
            continue

    # This block calculates Committed Capital and Fully Invested returns for the period
    start_trade_idx = past_days + twelve_months
    end_trade_idx = start_trade_idx + six_months
    
    # Slice the Rpair matrix for the current trading period
    Rpair_temp = Rpair[start_trade_idx:end_trade_idx, :]
    
    # Committed Capital (ew)
    wi_cc = np.ones((1, no_pairs))
    Rp_ew_cc.loc[start_trade_idx:end_trade_idx-1, 'Return'] = np.nansum(
        np.multiply(wi_cc, Rpair_temp), axis=1) / np.sum(wi_cc)
    Rp_ew_cc.loc[start_trade_idx:end_trade_idx-1, 'Semester'] = big_loop
    
    # Fully Invested (vw) - this requires knowing when pairs are open.
    # We will simulate this based on whether Rpair has non-zero returns.
    pa_open = (Rpair_temp != 0).astype(int)
    wi_fi = np.multiply(np.ones((1, no_pairs)), pa_open)
    
    # Handle days with no open pairs by assigning a return of 0
    sum_wi_fi = np.sum(wi_fi, axis=1)
    # Use np.divide to handle division by zero safely
    daily_fi_returns = np.divide(np.nansum(np.multiply(wi_fi, Rpair_temp), axis=1), sum_wi_fi, where=sum_wi_fi!=0)

    Rp_vw_fi.loc[start_trade_idx:end_trade_idx-1, 'Return'] = daily_fi_returns
    Rp_vw_fi.loc[start_trade_idx:end_trade_idx-1, 'Semester'] = big_loop
    
    # Store risk-free rates for the period
    risk_free_df.loc[start_trade_idx:end_trade_idx-1, 'Return'] = Rf.iloc[start_trade_idx:end_trade_idx, 0].values
    risk_free_df.loc[start_trade_idx:end_trade_idx-1, 'Semester'] = big_loop
...
df.to_csv("../cointegration_results/operations_v2.csv", sep=',', index=False)
...
pd.DataFrame(Rpair).to_csv("../cointegration_results/Rpair_v2.csv", header=None, index=False)
daily_returns = np.sum(Rpair, axis=1)
pd.DataFrame(daily_returns).to_csv("../cointegration_results/daily_returns_v2.csv", header=None, index=False)

print("\n\n========================================================")
print("           FINAL PORTFOLIO ANALYSIS")
print("========================================================")

# --- Helper functions (calculate_sortino_ratio, calculate_max_drawdown) are the same ---
# ... (Omitted for brevity) ...
...
for asset_ticker, count in most_common_assets:
    if asset_ticker in returns.columns:
        asset_returns = returns[asset_ticker].dropna()
        mean_ret, std_dev = asset_returns.mean(), asset_returns.std()
        sharpe_ratio = (mean_ret / std_dev) * np.sqrt(252) if std_dev > 0 else 0
        stats_data.append({
            "Symbol": asset_ticker, "Mean": f"{mean_ret:.4f}", "Std Dev": f"{std_dev:.4f}",
            "SR": f"{sharpe_ratio:.4f}", "Min": f"{asset_returns.min()*100:.2f}", "Max": f"{asset_returns.max()*100:.2f}"
        })
...
rf_daily = risk_free_df.loc[cc_returns.index, 'Return'].dropna()

cc_excess_returns = cc_returns - rf_daily
fi_excess_returns = fi_returns - rf_daily

# ... (Logic to calculate and print the performance metrics table is the same) ...

# --- Part 3: Cumulative Returns Plot ---
print("\n--- Generating Cumulative Excess Returns Plot ---")

if not cc_excess_returns.empty:
    cumulative_cc = (1 + cc_excess_returns).cumprod()
    x_axis_data = range(len(cumulative_cc))

    cumulative_cc_df = pd.DataFrame(cumulative_cc)
    cumulative_cc_df.to_csv(f"../cointegration_results/final_analysis_cum_return.csv")


    plt.style.use('seaborn-v0_8-whitegrid')
    fig, ax = plt.subplots(figsize=(14, 8))

    ax.plot(x_axis_data, cumulative_cc, label='Committed Capital', linewidth=2)

    ax.set_title('Cumulative Excess Returns (Committed Capital - Cointegration)', fontsize=16)
    ...
    fig = ax.get_figure()
    fig.savefig('./cointegration_cumm_return.png')
else:
